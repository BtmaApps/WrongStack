...
Session-scoped **subagent model lanes** sit above that matrix (`core/coordination/session-subagent-models.ts`). A session's plan is an ordered lane list plus an optional role overlay: each live subagent holds one lane and releases it on `Director.remove`, so N workers running at once land on N different provider/model pairs ("agent 1 … agent 8"). A role overlay is more specific than a lane — it wins and consumes none. `lock` (default on) decides whether a lane outranks the `provider`/`model` the leader passed to `spawn_subagent` / `delegate`; when it does, the leader's pins are discarded wholesale rather than half-applied, because a leader model beside a lane provider names a pair that exists in neither. `followSessionModel` ("use my model for all subagents") is the coarse form of the same statement and outranks the lanes. Both leave `/setmodel` routing intact — a role or phase the user routed there keeps its route, and only a session role override outranks it. The lock overrides the LEADER only: `spawn_subagent` / `delegate` stamp `SubagentConfig.modelChosenByLeader` when the model came from the leader's tool call, and a pin without that stamp (a person's `/spawn --model=…`, an ACP flag) outranks the lane. Full precedence at spawn: **session role override → `modelMatrix` role/phase route → human per-spawn pin → session lane or follow-session → leader pin → `modelMatrix` `*` → `modelTiers` → session model**. The plan lives in an in-memory registry keyed by session id and is journaled as a `subagent_model_plan` session event, so `/resume` restores it; it never touches config.json. Surfaces: `/subagent-models` (text + TUI lane panel), WebUI Settings → Routing → Subagent models, SimpleUI Settings → Session. All model resolution happens in exactly one place, `resolveDirectorSpawnModel`; `fleet-spawn.ts` deliberately carries none (guard: `spawn-model-single-resolver.test.ts`).
...
36 bundled skills live under `packages/core/skills/` (`output-standards` is depended on by almost every other skill — don't drop it). See `docs/skills.md`.
