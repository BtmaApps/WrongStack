...
| `WRONGSTACK_DESKTOP_IDLE_MINUTES` | Minutes a project may sit untouched in WrongStack Desktop before its `webui-server` child process is reclaimed (default `15`). Every open project holds one, so this is what keeps ten open projects from meaning ten Node processes. A project actively producing output is **never** reclaimed however long since you last looked at it, and the active project is never reclaimed at all; a reclaimed project stays in the sidebar as a stopped row and clicking it starts it again. `0` (or a negative value) keeps every project running for the whole session. |
| `WRONGSTACK_KANBAN_ARCHIVE_PURGE_DAYS` | Days after a session board is archived before it is **deleted**. Unset by default, which is why archived boards accumulate without a ceiling: a session mirror archives 7 days after its last update and the archive is then the surviving record of that session. Set a number (e.g. `90`) to trade that history for a bound. `0` or an unparseable value is treated as unset — it never means "delete now". |
