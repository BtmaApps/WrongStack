...
| #2 | HIGH | ✅ RESOLVED | `_internal.ts` | ~~`claimReadyTaskOnBoard()` returns `null` when `board.lifecycle.mode === 'managed'`, consistent with `assignTask()`.~~ **Superseded (2026-09-16):** the Phase 0-4 program below replaced that refusal with a managed branch that claims cards in the `todo` stage, preserves routing metadata and emits `task.claimed`. Managed claiming works; it is stage-gated rather than blocked. |
...
| #7 | MEDIUM | ✅ RESOLVED | `kanban-dispatch.ts` | `assignTask()` return value now checked in WebUI dispatch. Null returns are surfaced as clear error messages instead of silently continuing. **Superseded (2026-09-16):** `assignTask()` no longer has a managed-board branch at all — it writes the assignment and leaves status/column to the lifecycle, so a `null` now means only "task not found". |
