...
const logSpy = vi.hoisted(() =>
  vi.fn((...args: unknown[]) => {
    try {
      logLines.push(JSON.parse(String(args[0])));
    } catch {
      /* ignore */
    }
  }),
...
const { ProcessGuardian, startProcessGuardian, stopProcessGuardian } = await import(
  '../src/process-guardian.js'
);

describe('ProcessGuardian fatal handlers', () => {
  beforeEach(() => {
    // Root Vitest enables restoreMocks, so process-global spies must be
