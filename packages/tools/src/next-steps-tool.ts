...
export const nextStepsTool: Tool<NextStepsInput, NextStepsOutput> = {
  ...
  usageHint:
    ...
    "- The recipient is the LLM, not the user. Name the target, action, and useful verification or output in the user's language. Omit manual chores, approval requests, questions for the user, and mixed agent/human checklists.\n" +
        ...
        items: {
          ...
          properties: {
            text: {
              type: 'string',
              description:
                'A self-contained prompt submitted verbatim back to the LLM: target, action, and useful verification or output. Never advice, a question, or a manual chore addressed to the user.',
            },
            auto: {
              type: 'boolean',
              description:
  ...
  async execute(input, ctx) {
    ...
    const steps: PendingNextStep[] = input.steps
      .filter(
        (s): s is { text: string; auto?: boolean } =>
    ...
    // The slot preserves prompt text, applies the 1-item-only `auto` rule and cap, so
