...
export async function _httpJson<T>(
  ...
  const timer = setTimeout(
    () => controller.abort(),
    init?.timeoutMs ?? DEFAULT_TRANSPORT_TIMEOUT_MS,
  );
  try {
...
export async function createWrongTraceClient(
  ...
  const client: WrongTraceClientInternal = {
    ...
    async getFileHealth(path: string): Promise<WrongTraceFileHealth | null> {
      ...
      if (ipc.isWired) {
        const viaIpc = await ipc.call<WrongTraceFileHealth>('telemetry/file_health', {
          file_path: path,
        });
      ...
      return httpJson<WrongTraceFileHealth>(
        base,
        `/api/file/health?path=${encodeURIComponent(path)}`,
      );
    },

    async getSymbolLineage(path: string, signature?: string): Promise<WrongTraceSymbolEvent[]> {
      const base = requireBaseUrl();
      if (!base) return [];
    ...
    async getAtlas(query?: WrongTraceAtlasQuery): Promise<WrongTraceAtlasSummary | null> {
      ...
      if (ipc.isWired) {
        const viaIpc = await ipc.call<WrongTraceAtlasSummary>('get_atlas', {
          ...(query?.workspace !== undefined && query.workspace !== ''
