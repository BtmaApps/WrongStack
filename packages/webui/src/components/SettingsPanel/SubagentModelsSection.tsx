    ...
    <div className="pt-2">
          ...
          return (
            ...
            >
              ...
              <button
                ...
                className="h-8 min-w-0 flex-1 truncate rounded-md border bg-background px-2 text-left font-mono text-xs hover:border-primary/40 disabled:cursor-not-allowed"
              >
      ...
      {pickerLane !== null && (
        <SubagentModelPickerDialog
          ...
          currentLane={pickerLaneValue ?? {}}
          open
          onOpenChange={(next) => {
            if (!next) setPickerLane(null);
          }}
          onPick={(pick) => {
            setLane(pickerLane, pick ? { provider: pick.provider, model: pick.model } : {});
            setPickerLane(null);
          }}
        />
      )}
