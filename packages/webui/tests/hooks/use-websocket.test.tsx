...
function makeClient() {
  ...
  const client = {
    ...
    getSystemPrompt: record('getSystemPrompt'),
    send: record('send'),
    ...
    // ── Domain methods mirroring src/lib/ws-client-domain-methods.ts ──
    // Keep this block in sync with the registry so bootstrap/handler code
    // never calls a method the stub lacks (the getSystemPrompt drift of
    // 2026-09-23 logged webui.ws_connection_failed on every run).
    getGitInfo: record('getGitInfo'),
    getGitChanges: record('getGitChanges'),
    getGitHistory: record('getGitHistory'),
    getGitCommitDetail: record('getGitCommitDetail'),
    getGitCommitFileDiff: record('getGitCommitFileDiff'),
    getGitDiff: record('getGitDiff'),
    stageGit: record('stageGit'),
    unstageGit: record('unstageGit'),
    discardGit: record('discardGit'),
    commitGit: record('commitGit'),
    getProviderQuota: record('getProviderQuota'),
    getProviderStatus: record('getProviderStatus'),
    getProviderAuditHistory: record('getProviderAuditHistory'),
    retryProviderModel: record('retryProviderModel'),
    clearProviderStatus: record('clearProviderStatus'),
    newSession: record('newSession'),
    setSystemPromptVariant: record('setSystemPromptVariant'),
    listProviders: record('listProviders'),
    listProviderModels: record('listProviderModels'),
    listSavedProviders: record('listSavedProviders'),
    searchProviderModels: record('searchProviderModels'),
    addKey: record('addKey'),
    updateKey: record('updateKey'),
    deleteKey: record('deleteKey'),
    setActiveKey: record('setActiveKey'),
    addProvider: record('addProvider'),
    removeProvider: record('removeProvider'),
    listOAuthProviders: record('listOAuthProviders'),
    startOAuth: record('startOAuth'),
    submitOAuthCode: record('submitOAuthCode'),
    cancelOAuth: record('cancelOAuth'),
    probeProvider: record('probeProvider'),
    clearProviderModels: record('clearProviderModels'),
    undoProviderClear: record('undoProviderClear'),
    setCustomModel: record('setCustomModel'),
    removeCustomModel: record('removeCustomModel'),
    updateProvider: record('updateProvider'),
    clearContext: record('clearContext'),
    compactContext: record('compactContext'),
    repairContext: record('repairContext'),
    openContextEditor: record('openContextEditor'),
    validateContextEditor: record('validateContextEditor'),
    applyContextEditor: record('applyContextEditor'),
    debugContext: record('debugContext'),
    listContextModes: record('listContextModes'),
    switchContextMode: record('switchContextMode'),
    createContextMode: record('createContextMode'),
    updateContextMode: record('updateContextMode'),
    deleteContextMode: record('deleteContextMode'),
  };
  return client;
}
