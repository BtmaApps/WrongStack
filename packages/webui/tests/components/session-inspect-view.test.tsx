...
describe('SessionInspectView — loading → payload transition', () => {
  ...
  it('does not throw across the loading → payload transition', async () => {
    ...
    expect(
      screen.getByText(/(?:sessionInspect\.noSessionSelected|No session selected)/i),
    ).toBeTruthy();
    ...
    expect(
      screen.getByText(
        /(?:activity:)?sessionInspect\.loadingSessionInspection|Loading session inspection/i,
      ),
    ).toBeTruthy();
  ...
  it('renders only matching events when the filter input has a value', async () => {
    ...
    const filterInput = screen.getByPlaceholderText(
      /(?:sessionInspect\.filterEvents|Filter events)/i,
    );
