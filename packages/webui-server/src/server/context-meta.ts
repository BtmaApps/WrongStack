...
export function seedContextMeta(config: Config, context: { meta: Record<string, unknown> }): void {
  ...
  meta['enhanceDelayMs'] = (autonomyCfg['enhanceDelayMs'] as number) ?? 15_000;
  ...
  meta['logLevel'] = config.log?.level ?? 'warn';
  meta['auditLevel'] = config.session?.auditLevel ?? 'full';
  meta['maxIterations'] = config.tools?.maxIterations ?? 0;
  meta['contextMode'] = config.context?.mode ?? 'balanced';
  meta['systemPromptVariant'] = config.systemPrompt?.variant ?? 'pro';
  {
