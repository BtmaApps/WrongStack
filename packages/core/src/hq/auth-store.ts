...
import { hashHqPassword, mintHqCookieSecret, verifyHqPassword } from './auth-passwords.js';
...
export async function writeHqAuthFile(dataDir: string, file: HqAuthFile): Promise<void> {
  ...
  // Harden the DIRECTORY too, not just the file.
  //
  // `auth.json` holds the password hash, the cookie secret, the TOTP secret
  // and the recovery-code hashes, and it is written `0600`. Its parent was
  // left at whatever `mkdir` produced — `0755` under a normal umask. The file
  // mode means another local account cannot read the secrets directly, but a
  // world-readable parent lets it enumerate the directory, and any file
  // written there later by a path that does not set its own mode inherits the
  // permissive default.
  //
  // `restrictDirPermissions` was written for exactly this and had no
  // production call site at all (audit 2026-08-20, re-reported 2026-09-10) —
  // a correct helper that nothing reached, which is this codebase's most
  // common security defect by a wide margin. Best-effort like its file
  // sibling: a hardening step must never fail the write it protects.
  await restrictDirPermissions(dataDir, { label: 'hq-auth-dir' });
}
