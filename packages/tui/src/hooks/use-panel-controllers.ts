...
export function usePanelControllers({
  ...
  const openSettings = React.useCallback(() => {
    ...
    dispatch({
      ...
      nextPrediction: s.nextPrediction ?? true,
      ...
      autoProceedMaxIterations: s.autoProceedMaxIterations ?? 0,
      enhanceDelayMs: s.enhanceDelayMs ?? 15_000,
      enhanceEnabled: s.enhanceEnabled ?? true,
      enhanceLanguage: (s.enhanceLanguage as 'original' | 'english') ?? 'english',
      debugStream: s.debugStream ?? true,
