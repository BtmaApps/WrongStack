...
export function routeSettingsOverlayKey(
  ...
  dispatch({
    ...
    chime: config.chime ?? true,
    confirmExit: config.confirmExit ?? true,
    nextPrediction: config.nextPrediction ?? true,
    ...
    autoProceedMaxIterations: config.autoProceedMaxIterations ?? 0,
    enhanceDelayMs: config.enhanceDelayMs ?? 15_000,
    enhanceEnabled: config.enhanceEnabled ?? true,
    enhanceLanguage: config.enhanceLanguage ?? 'english',
    debugStream: config.debugStream ?? true,
