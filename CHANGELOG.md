...
- **Request middleware installed after the model-runtime settings never ran.** The `ModelRuntimeSettings` middleware returned its request instead of passing it on, so everything after it in the request pipeline was skipped on every turn. That covered all of SAGE's request side: use credit (every memory's `useCount` stayed 0; a store of 14,000 memories had only `memory.injected` audits), domain-term extraction, and opt-in turn-context injection. `@skill` mentions and skill suggestions in the CLI were skipped too. The middleware now comes from one core factory (`createModelRuntimeMiddleware`) used by the leader and both subagent factories. Verified with a real CLI turn: a file-anchored memory the answer quoted got `useCount: 1` and a `memory.used` audit.
- **A provider built through the catalog lost its own name.** Every host asks the registry for a provider by its config key (`{ ...cfg, type: providerId }`, with the factory type apart), but the catalog factories named it after the catalog entry and the generic `openai-compatible` factory always `openai-compatible`. So a second account `work` (type `anthropic`) ran as `anthropic`, and `my-local` as `openai-compatible`. Everything that reads the running provider's own config entry got another one: key hot-reload, custom model definitions in the system prompt, vision detection, per-model limits. The provider now carries its config key; how it behaves (wire format, routing, presets) is still decided by its catalog entry and factory, and catalog lookups resolve an alias to its entry. A directory rule's `denyProviders` still covers such an account when it names the vendor (`anthropic`), as it did before, as well as when it names the account. Two related fixes: the WebUI's Brain chain built an alias by looking up a factory named after the alias, and an SDD subagent's provider was named after its factory.
- **A plugin middleware's crash failed the whole turn in the standalone WebUI.** Its pipelines had no error boundary, so the error the CLI logs and skips reached the run. Both hosts now install the same boundary from core (`installPipelineErrorBoundaries`): a plugin's crash is logged, reported as an `error` event and skipped; a core one still fails the run.
- **The standalone WebUI ignored the model-runtime settings.** Its pipelines were built without the model-runtime middleware, so the reasoning mode and effort, cache TTL and generic parameters chosen in its settings were saved and never reached a request. It now installs the middleware, and a setting saved while running applies to the next request.
- **A WebUI tab on its own model got the leader's reasoning profile.** In the standalone WebUI and the CLI's `--webui`, a tab can run a different model than the one the process booted on. The reasoning settings were checked against the boot model's capabilities, so a tab could get a field its model rejects, or lose one it supports. Each request is now checked against the model it goes to.
- **A model with an unknown context window could not be used in the standalone WebUI.** Auto-compaction measured the request against a window of 0, read every send as over the hard line and refused it ("Auto-compaction did not reduce context below hard threshold"). With the window unknown it now lets the send through, as the CLI already did by not installing it.
- **The standalone WebUI refused a keyless local server of type `openai-compatible`.** The generic `openai-compatible` factory still demanded an API key where boot, the picker and the named-provider path accept a loopback server that names no key variable.
- **MCP tools never reached the model with token saving off.** Tier `off` kept the browser suite off the provider surface by fixing the surface to the tool names registered at boot, which also hid every tool registered afterwards, MCP servers' included. It is now an exclusion, so later tools are offered.
- **Vision models on OpenAI-compatible providers were treated as text-only.** The catalog's "takes images" fact was AND-ed with the `openai-compatible` family default, which is no vision so that an unknown model is not sent images. That hid vision on 4,236 of the 4,754 image-input models in the catalog, among them Z.AI's GLM-4.5V/4.6V, Llama 4, Gemma 3 and every vision model on OpenRouter, xAI, Mistral and DeepInfra. An attached image was refused with "does not support image input … enable an MCP/tool adapter", or detoured through a vision MCP. The family default now applies only when the catalog says nothing, because the transport sends images as `image_url` parts. Only providers without a supported transport are still held to no vision.
- **A one-shot run (`wstack "task"`) started before its MCP servers were up.** Servers start in the background and a single turn takes the tool list as it is when it starts, so their tools were missing from it. A one-shot run now waits for the in-flight starts to connect or fail first (each start is bounded by the client's own startup timeout).
...
- **MCP URL-mode elicitation (spec 2025-11-25).** A server can ask the user to open a web page for something that must not pass through WrongStack — a third-party sign-in, an API key, a payment. The client declares `elicitation: { form: {}, url: {} }`.
  - **Consent first.** The request is shown as a form naming the server, with the full URL, the site, and warnings for a punycode host, plain HTTP off localhost, or credentials in the address. The choices are "open it in the browser on <machine>" (only where the host can open one), "I will open it myself" (the browser may be elsewhere, as with `wstack remote`), or decline. Nothing is fetched or opened before that, and only `http(s)` URLs are accepted.
  - **`-32042` (URL elicitation required).** A tool call the server refuses this way puts each listed page to the same user; the model is told which pages, what the user chose, and whether to call again.
  - **Verified** in a real WebUI against a real stdio MCP server: the form showed the URL and the site, "I will open it myself" reached the server as `accept`, and a run with no one to ask answered `cancel` without opening anything.
- **A form nobody answers during eternal or parallel autonomy no longer holds the run.** A `clarify` form or an MCP elicitation left unanswered for the tool-approval wait (120 s) while the conversation's autonomy mode is `eternal` / `eternal-parallel` is closed on every surface: `clarify` takes its recommended answers, an elicitation is cancelled. Other modes keep waiting for the person at the keyboard. The rule for "which mode is this conversation in" (its own mode first, the process-wide one as the fallback) is now one function shared with the system-prompt builder. HQ accepts the new `unattended` resolution source.

- **`wstack remote user@host:/path`: WrongStack on another machine, its WebUI here.** The agent, its tools and its project daemons run on the remote machine, in that directory; the WebUI opens locally through an SSH tunnel.
  - **The system `ssh`**, so `~/.ssh/config`, the SSH agent and `known_hosts` apply. `--ssh-config <file>` passes `-F`. Without a terminal (and without `SSH_ASKPASS`) it runs in `BatchMode`, so a password prompt cannot hang it.
  - **The build is uploaded once per version** to `~/.wrongstack/remote/` and its SHA-256 checked on arrival. It is this binary when the remote machine has the same target, otherwise a release download verified like `wstack update` (cached under `remote-builds/`), or `--remote-binary <file>`. Linux (glibc or musl) and macOS, x64 and arm64.
  - **The remote host outlives the connection.** It starts detached on the remote loopback; its token is passed on stdin, never on a command line, and kept in a `0600` file. A dropped tunnel is reopened and the same host reused. Ctrl+C stops it, `--keep` leaves it running for the next `wstack remote`, which reuses it when the version matches and replaces it when not.
  - **Nothing local is copied.** A remote machine with no usable provider starts in setup mode, and the provider is chosen in its WebUI.
  - **Verified** against a real sshd in a container: upload and reuse, a real turn whose tool ran remotely, reconnect after the tunnel process was killed, Ctrl+C stopping the remote host, and the release-download path with the published 1.0.25 build. An sshd that refuses port forwarding (Alpine's default) now fails with a message naming `AllowTcpForwarding` instead of waiting silently.

- **Code mode: `tool_script`.** The model can write one short JavaScript program that calls tools as async functions (`await tools.read({ path })`, `tools.call('codebase-search', input)`), loops over and filters their results, and returns only the final value. A chain like "read ten files and count something" is one step, and the intermediate output never enters the conversation.
  - **The sandbox is QuickJS compiled to WebAssembly.** It has no filesystem, network or process access and no `require` or `import`; a tool call is the only way out.
  - **Every call goes through the agent's own gate** (`ctx.nestedToolCall`), the one that direct calls use: validation, hooks, the permission check, the user confirmation, the `tool.executed` event and the journal (`tool_use` / `tool_result`, id `<script call>~<n>`). The script tool itself is `auto` and not mutating, so running it asks nothing; each call it makes is asked about as that call.
  - **What stops a script.** Its wall-clock timeout (120 s unless `timeout_ms` says otherwise; `0` = none, and the executor's generic tool ceiling does not cut it short), the user interrupting the run, 5 s of computation without pausing at an `await` (the VM shares the host's event loop, so an endless loop would otherwise freeze the process past any abort), and the 64 MiB tool memory guard. There is no call limit unless the model sets `max_calls`, and nothing the script returns or logs is cut: large results are spooled like any tool's output. A script cannot run another script, directly or through `tool_use`. `Promise.all` runs calls side by side; `console.log` output comes back with the result.
  - **Tools see their own call id.** `execute()` gets `toolUseId`, so a tool can label what it does on behalf of that call.
  - **Verified.** A real CLI run read three files through one script, three reads in parallel, and returned the right count; the reads were journaled under the script's call. The same script ran in the compiled Windows binary. Tests cover the confirmation of a call made from a script, the refusal reaching the script as an error, and each limit. (`packages/tools/src/tool-script.ts`, `packages/core/src/core/agent-tools.ts`)

- **One prompt queue per session, shared by the terminal and the browser.**
  - **One file.** The WebUI host keeps a session's queue in the session's own folder (`queue.json`), where the TUI keeps its queue, and in the same shape. A prompt queued in either surface and left behind is there when the session is opened in the other. The WebUI runs it at once; the TUI shows it and runs it after the next turn. Queues kept in the old `prompt-queue/` folder move over the first time their session is read.
  - **`/resume` in the TUI brings the session's queue along.** It used to empty the queue and write that into the session you were leaving, deleting its saved queue, and it never loaded the queue of the session you resumed.
  - **Queued attachments.** A queued prompt with images or PDFs is stored as its content blocks and runs as them, so it runs the same wherever it drains.
  - **Verified in the real TUI and WebUI**, in all three directions. A prompt queued in the WebUI during a turn, with the host then killed, was restored by `wstack -r` and ran after the next prompt. A prompt queued in the TUI during a long tool run, with the TUI then closed, ran as soon as the session was opened in the WebUI. `/resume` into a session with a queued prompt showed it and ran it after the next turn. (`packages/webui-server/src/server/session-prompt-queue.ts`, `packages/tui/src/hooks/use-queue-manager.ts`)

- **Jujutsu and Mercurial projects.** A version-control layer in core (`vcs/`) gives git, jj and hg one interface. It reports the commit the working copy is based on, the paths changed since, whether the checkout is untouched, and the branch or bookmark. It also runs every VCS binary the same bounded way.
  - **Workspace checkpoints** now work in jj and hg repositories. They record `jj-head-plus-dirty` / `hg-head-plus-dirty` against the base commit, where they used to be skipped.
  - **The model's environment line** says `Jujutsu (jj): bookmark=trunk, 4 changed` or `Mercurial (hg): branch=default, 5 changed` instead of "not a git repo".
  - **Mercurial ignore rules.** In a Mercurial repository WrongStack keeps its own state out of `hg status` through `.hgignore`, as it already does through `.gitignore`. Mercurial does not read `.gitignore`, so the rules are added as per-line `re:` rules.
  - **Detection** finds the nearest repository. A colocated jj repository is handled as git. Worktrees stay git-only.
  - **Verified** with jj 0.45.1 and Mercurial 7.2.4. In a non-colocated jj repository and an hg repository, a real CLI run captured checkpoints of the right base commit, containing only the project files, and the model quoted the new environment line. The adapter tests run against real jj and hg when they are installed. (`packages/core/src/vcs/`, `packages/core/src/storage/session-checkpoint-cas.ts`, `packages/core/src/utils/project-identity.ts`)

- **A native Windows on ARM binary.** Releases now include `wstack-windows-arm64.exe`, built with Bun's `bun-windows-arm64` target and smoke-tested on a `windows-11-arm` runner. `install.ps1` picks it on an ARM64 machine, even when PowerShell itself runs as an x64 process. For a release that has no arm64 entry in `SHA256SUMS`, it installs the x64 build, which runs under emulation. A native build updates itself to the arm64 asset. Verified locally: the full CLI compiles to an ARM64 executable with its build target embedded, and the installer, run against a local mirror, chose the right file for x64, for ARM64 with a new release, and for ARM64 with an older release. It refused a file whose checksum did not match. (`scripts/build-binaries.mjs`, `scripts/install/install.ps1`, `.github/workflows/binaries.yml`)
...
- **Rewind or fork from a message in the TUI, and the prompt comes back to the composer.**
  - **From a message.** Mark one of your messages with Alt+↑ / Alt+↓ and press Enter: the rewind timeline opens on the checkpoint of that message. Enter rewinds to it; `f` forks a new session from just before it. Esc, or any other key, drops the mark.
  - **Fork.** The fork keeps the conversation up to that message and switches to it; the original session is unchanged, and both still share the working tree. `SessionStore.fork` takes `beforeCheckpointPrompt` for this; `wstack sessions fork --checkpoint N` still forks just after prompt N.
  - **The prompt comes back.** After a rewind or a fork, the prompt that was taken back is put in the composer, ready to be sent again or changed. A draft you had typed is kept instead, and the prompt is shown above it.
  - **The transcript is redrawn.** A rewind, and `/rewind redo`, now show the turns that remain instead of an empty screen.
  - **Verified in the real TUI** with two prompts: Alt+↑ and Enter opened the timeline on the second prompt's checkpoint, the rewind left the first turn on screen with the second prompt in the composer, and Enter sent it again as checkpoint #1. Then Alt+↑ twice, Enter, `f` forked at the first prompt: the fork started empty with that prompt in the composer. Both journals reload with each prompt once. (`packages/tui/src/message-checkpoint.ts`, `packages/tui/src/rewind-prompt.ts`, `packages/tui/src/hooks/fork-at-checkpoint.ts`, `packages/core/src/storage/session-store/fork-session.ts`)
...
- **A keyless server on a loopback address could be saved as the default but not started.** Boot and the picker accept a provider whose base URL is `localhost` / `127.x` / `::1` and that names no key variable (a local OpenAI-compatible server) as usable; the provider factory still demanded an API key, so the next start failed with "requires an API key". The rule now lives in core (`isKeylessLocalProvider`) and all three use it; such a provider is built with the `no-key` placeholder the local presets send. Verified with a real CLI turn against a loopback server with no key configured.
- **`tool_script` carried limits of its own.** It capped a script at 50 tool calls (at most 200), 600 s, 5 s of total computation and 100,000 characters, and cut its result at 60,000 characters and its console at 8,000. It now has no call limit unless the model sets `max_calls`, returns everything (large output is spooled like any tool's), accepts `timeout_ms: 0`, and manages its own timeout: the executor's generic 300 s tool ceiling used to cut a longer `timeout_ms` short. What remains is the wall-clock timeout, 5 s of computation *without pausing at an await* (the VM shares the host's event loop, which an endless loop would otherwise freeze past any abort), and the 64 MiB tool memory guard.
- **A session that had just been created could be deleted as "old".** The session catalog records a transcript's modification time as 0 when the session is recorded before its transcript reaches disk. If the process then died, the next start's clean-up read 0 as older than 30 days and deleted the session, with anything its folder held, such as a queued prompt. Such a row is now aged from when it was recorded. (`packages/core/src/session-catalog/store.ts`)
- **A project in a subdirectory of a git repository was "not a git repo".** The environment line looked only for a `.git` directory in the project folder. It now finds the repository above. Workspace checkpoints of such a project had also recorded files relative to the repository root but restored them relative to the project. They now keep project-relative paths and leave files outside the project out. (`packages/core/src/core/system-prompt-environment.ts`, `packages/core/src/storage/session-checkpoint-cas.ts`)
- **A git repository with no commit yet reported `branch=No`.** Git's `## No commits yet on main` header was read as the branch name. (`packages/core/src/core/system-prompt-environment-probes.ts`)
- **A rewind did not take the prompt back from the model.** The agent journals a prompt before it writes that prompt's checkpoint, and a rewind cuts the journal just after the checkpoint. The rewound prompt was therefore reloaded into the conversation. The model kept seeing it, and the prompt sent again after the rewind followed it as a second copy, under the wrong checkpoint number. A `rewound` event now takes the conversation back to where it stood before that prompt. (`packages/core/src/storage/session-store/load-session-data.ts`)
- **A session without a conversation snapshot loaded its first prompt twice.** A one-turn session, or the part of a journal a rewind kept, rebuilt the first prompt both from its `user_input` event and from the message journaled right after it. The two are now read as one message. (`packages/core/src/storage/session-store/load-session-data.ts`)
- **The TUI's `/rewind` timeline was always empty.** The runtime built the session store without the event bus, so `checkpoint.written` and `session.rewound` were never published. The timeline listed nothing, `/rewind` with no number had nothing to show, and the governance shadow bridge never saw a rewind. (`packages/runtime/src/container.ts`, `packages/cli/src/boot/tui-project-switch.ts`)
- **A rewind in the TUI ran `/clear`.** That emptied the checkpoint list and dropped pending attachments, and the draft in the composer was lost. Now a rewind only clears the screen, which is redrawn, and the draft is put back. (`packages/tui/src/hooks/use-tui-event-bridge.ts`, `packages/tui/src/rewind-prompt.ts`)
- **The timeline showed the rewound prompt for its checkpoint.** A prompt sent after a rewind reuses the checkpoint number, and the timeline kept the old row. It now shows the new prompt. (`packages/tui/src/reducers/workspace-panels.ts`)
